export default function Nav() {
  return (
    <nav className="w-screen mx-auto md:w-full shadow" style={{position: "sticky", top: 0, background: "#ffffff"}}>
    <div className="py-2 flex justify-between">
    <a href="/"><img className="h-16" src="/assets/logo.png" /></a>
    <div class="block lg:hidden">
    <button className="flex items-center px-3 py-2 mt-4 border rounded text-gray-500 border-gray-600 hover:text-gray-800 hover:border-teal-500 appearance-none focus:outline-none">
    <svg className="fill-current h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    <title>Menu</title>
    <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
    </svg>
    </button>
    </div>
    <div className="hidden lg:block">
    <ul className="inline-flex my-5">
    <div className="dropdown hover:bg-green-500">
      <button className="px-4 hover:text-gray-800 text-base uppercase" href="#">Genesis</button>
        <div class="dropdown-content">
          <a href="/problem">Problem</a>
          <a href="/learning">Learning</a>
        </div>
    </div>

    <div className="dropdown hover:bg-green-500">
      <button className="px-4 hover:text-gray-800 text-base uppercase" href="#">Methodology</button>
        <div class="dropdown-content">
          <a href="/approach">Approach</a>
          <a href="/platform">Platform</a>
        </div>
    </div>

    <div className="hover:bg-green-500">
      <a className="px-4 hover:text-gray-800 text-base uppercase" href="/collaboration">Collaboration</a>
        {/* <div class="dropdown-content">
          <a href="/problem">Approach</a>
          <a href="/learning">Platform</a>
        </div> */}
    </div>

    <div className="dropdown hover:bg-green-500">
      <button className="px-4 hover:text-gray-800 text-base uppercase" href="#">About</button>
        <div class="dropdown-content">
          <a href="/team">Team</a>
          <a href="/partners">Partners</a>
        </div>
    </div>

    </ul>
    </div>
    </div>
    </nav>
    )
  }